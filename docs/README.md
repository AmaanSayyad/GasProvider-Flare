# Gas Provider documentation

**Flare Summer Signal · Track 1 (Interoperable Asset Products)**

Pay FXRP/C2FLR on Coston2 → native gas on destination chains via FTSO pricing, FDC attestation, and pre-funded treasuries.

## Index

### Users
- [USER_GUIDE.md](./USER_GUIDE.md) — wallet, deposit, tracking

### Developers
- [DEPLOYMENT_GUIDE.md](./DEPLOYMENT_GUIDE.md) — contracts, backend, frontend  
- [ENVIRONMENT_VARIABLES.md](./ENVIRONMENT_VARIABLES.md) — env reference  
- [FLARE_INTEGRATION_FILES.md](./FLARE_INTEGRATION_FILES.md) — FTSO / FDC / FAssets map  

### Operators
- [TREASURY_ADDRESSES.md](./TREASURY_ADDRESSES.md) — deployed treasuries  
- [EXCHANGE_RATE_CONFIGURATION.md](./EXCHANGE_RATE_CONFIGURATION.md) — fallback rates  

### Context
- [BRIDGE_COMPARISON.md](./BRIDGE_COMPARISON.md) — why treasury dispersal ≠ DEX bridge  

## Quick treasuries

- **Coston2:** `0xc031c437d6b915dbdc946dbd8613a1ac9dd75d63`  
- **Other demo testnets:** `0x5b402676535a3ba75c851c14e1e249a4257d2265`  

## Live links

- Frontend: https://gas-provider.vercel.app  
- Backend API: https://backend-production-6f62.up.railway.app  
- Pitch deck: https://docs.google.com/presentation/d/1dcDHlryNzrDfVudiqPB-xnrNiIX9w971PFaoAP4Jztk/edit?usp=sharing  
- Flare docs: https://dev.flare.network/  
